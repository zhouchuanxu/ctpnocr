...
def gen_anchor(featuresize, scale):
    ...
    base_anchor = np.array([0, 0, 15, 15])      #原始图像和现在图像比例为16：1
    # center x,y
    xt = (base_anchor[0] + base_anchor[2]) * 0.5    #anchor的中心坐标
    ...
    h, w = featuresize
    shift_x = np.arange(0, w) * scale  #特征图比例没用，映射到原始图像中*16
    ...
    return np.array(anchor).reshape((-1, 4))            #得到的候选框5600个
...
def cal_rpn(imgsize, featuresize, scale, gtboxes):
    ...
    # gen base anchor
    base_anchor = gen_anchor(featuresize, scale)    #得到所有的候选框 后面会舍弃和标注不太相近的框

    # calculate iou
    overlaps = cal_overlaps(base_anchor, gtboxes)   #计算anchor与标注iou进行舍弃选择 重合度大的拿出来

    # init labels -1 don't care  0 is negative  1 is positive
    labels = np.empty(base_anchor.shape[0])     #越界变成-1直接不要了
    labels.fill(-1)
    ...
    labels[outside_anchor] = -1     #越界的预选框全部为-1

    # subsample positive labels ,if greater than RPN_POSITIVE_NUM(default 128)
    fg_index = np.where(labels == 1)[0]     #预选框的个数
    # print(len(fg_index))
    if (len(fg_index) > RPN_POSITIVE_NUM):      #预选框太多了 就选150个
    ...
    # debug here
    bbox_targets = bbox_transfrom(base_anchor, gtboxes[anchor_argmax_overlaps, :])  #微调h和y  接近更加真实的值
    # bbox_targets=[]
